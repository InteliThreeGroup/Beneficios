...
";
  ...
  // Função para extrair o texto da resposta JSON do Gemini
  private func extractTextFromGeminiResponse(jsonResponse: Text) : Text {
    // Procura pelo padrão "text": "..." na resposta JSON
    let parts = Text.split(jsonResponse, #text "\"text\": \"");
    switch (parts.next()) {
      case (null) { "Erro: Não foi possível encontrar texto na resposta" };
      case (?_) {
        switch (parts.next()) {
          case (null) { "Erro: Formato de resposta inválido" };
          case (?textPart) {
            // Encontra o final da string de texto (próxima aspas duplas não escapada)
            let endParts = Text.split(textPart, #text "\"");
            switch (endParts.next()) {
              case (null) { "Erro: Não foi possível extrair o texto" };
              case (?extractedText) { 
                // Remove caracteres de escape \n para quebras de linha reais
                Text.replace(extractedText, #text "\\n", "\n")
              };
            };
          };
        };
  ...
  private func callGeminiAPI(prompt: Text) : async Text {
    ...
    let apiKey = "AIzaSyCvvro8v2NE16jLLmpfJ0vzFNr5zIcMLWI";
    ...
    switch (Text.decodeUtf8(response.body)) {
      case (?text) { 
        // Extrai apenas o texto da resposta em vez de retornar todo o JSON
